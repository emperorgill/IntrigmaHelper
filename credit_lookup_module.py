...
# In retrospect, I probably should have just hard-coded the configuration
# file here as constants...
...
def credit_lookup(shift_name, start_time, end_time):
    ...
    Weekend Credits, and Total Credits.  Return all of these as a tuple."""
    # Sensibility checking...
    if start_time > end_time:
        print("Error in credit_lookup_module/credit_lookup!  Start_time of " +
              str(start_time)+ " is greater than end time of " + str(end_time))
        sys.exit(1)
    ...
    # These are the variables we'll be returning to the calling function
    early_credits = 0
    late_credits = 0
    night_credits = 0
    call_credits = 0
    ...
    if shift_name in no_credit_shifts_list:
        return (early_credits, late_credits, night_credits, call_credits,
                friday_credits, weekend_credits, total_credits)
    ...
    # Return all calculated credits to the calling function.
    total_credits = early_credits + late_credits + night_credits + \
        call_credits + friday_credits + weekend_credits
    return (early_credits, late_credits, night_credits, call_credits,
            friday_credits, weekend_credits, total_credits)        
...
def read_config_file(filename = "Config Files/Credit Scoring.txt"):
    ...
    try:
        ...
        print("Error in credit_lookup_module/read_config_file - could not" +
              " open " + filename)
        sys.exit(1)
    ...
    file_contents, friday_credits = get_typea_list(file_contents,
            "Friday shifts\n", "End Friday shifts\n")
    file_contents.remove("End Friday shifts\n")
...
def get_typea_list(file_contents, first_delimiter, second_delimiter):
    """Processes a "Type A list" from the configuration file.  The list will
    be found between the first and second delimiters.  The list will have a 
    shift on each line.  First is the time (a string in 24 hour format)
    and then is a tab and then is the shift credits (a float in string format).
    Process these into a dictionary and remove the lines that were processed
    from file_contents.  Return the results."""
    output_dict = {}
    temp = []
    # Get the part of file_contents with the early shift credit info.  Toss the
    # line that says,"Early shifts".  Stop when we hit the line that says 
    # "Late shifts".
    for line in file_contents:
        if line == first_delimiter:
            continue
        if (line != first_delimiter and line != second_delimiter):
            temp.append(line)
        if line == second_delimiter:
            break
...
def get_typeb_list(file_contents):
    """Processes a "Type B list" from the configuration file.  This list is a
    just a line of comma separated strings.  Grab, process, and return the
    first such list in file_contents.  Remove that list from file_contents and
    return what remains of file_contents as well."""
    output_list = []
    temp = file_contents[0]
    output_list = temp.strip().split(",")
    file_contents.remove(temp)
